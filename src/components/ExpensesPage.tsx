...
export const ExpensesPage = () => {
  ...
  useEffect(() => {
    ...
    syncTransactions();
    return subscribeToStorageSync(syncTransactions);
  }, []);
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex gap-4">
              ...
              <div className="flex gap-2">
                <select
                  value={sortBy}
                  onChange={(event) => setSortBy(event.target.value as 'date' | 'amount' | 'category')}
            ...
            {(['all', 'expense', 'income'] as const).map((value) => (
              <button
                ...
                className={cn(
                  ...
                  filter === value ? 'bg-primary text-on-primary' : 'bg-surface-container text-on-surface-variant',
                )}
              >
                {value}
              </button>
          ...
          <div className="space-y-3">
            <AnimatePresence mode="popLayout">
              {filtered.map((transaction) => {
                const amountFormatted = formatDualCurrency(transaction.amount);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      >
                        {transaction.type === 'income' ? <ArrowDownLeft className="w-6 h-6" /> : <ArrowUpRight className="w-6 h-6" />}
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <p className="font-black text-on-surface tracking-tight">{transaction.description || transaction.category}</p>
                          <span
                            className={cn(
                      ...
                      <div className="text-right">
                        <p
                          className={cn(
                            ...
                            transaction.type === 'income' ? 'text-emerald-400' : 'text-on-surface',
                          )}
                        >
                          {transaction.type === 'income' ? '+' : '-'}
                          {amountFormatted.main}
                        </p>
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-end md:items-center justify-center p-4">
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} className="bg-surface w-full max-w-md rounded-t-3xl md:rounded-3xl p-8 space-y-6 border-t border-outline-variant/20">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <input
                ...
                required
                value={newTx.amount}
                onChange={(event) => setNewTx({ ...newTx, amount: event.target.value })}
              ...
              <input
                ...
                onChange={(event) => setNewTx({ ...newTx, description: event.target.value })}
              ...
              <select
                ...
                onChange={(event) => setNewTx({ ...newTx, category: event.target.value })}
              ...
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setShowAdd(false)} className="flex-1 py-4 rounded-xl font-bold text-on-surface-variant hover:bg-white/5">
                  Cancel
                </button>
                <button type="submit" className="flex-1 py-4 rounded-xl font-bold bg-primary text-on-primary">
                  Save
                </button>
              </div>
            </form>
