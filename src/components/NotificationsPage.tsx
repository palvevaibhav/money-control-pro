...
import { Bell, Check, CheckCircle2, Info, AlertTriangle, Trash2 } from 'lucide-react';
...
export const NotificationsPage = () => {
  ...
  useEffect(() => {
    ...
    syncNotifications();
    return subscribeToStorageSync(syncNotifications);
  }, []);
  ...
  const deleteNotification = (event: React.MouseEvent, id: string) => {
    event.stopPropagation();
    storage.deleteNotification(id);
  };

  const clearAll = () => {
    storage.clearNotificationsForCurrentUser();
  };
    ...
    <div className="space-y-8 pb-24">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mt-1">System Insights & Alerts</p>
        </div>
        <div className="flex gap-2">
          <button onClick={markAllAsRead} className="p-3 bg-surface-container rounded-2xl text-primary hover:bg-primary/10 transition-colors" title="Mark all as read">
            <Check className="w-5 h-5" />
          </button>
          <button onClick={clearAll} className="p-3 bg-surface-container rounded-2xl text-rose-500 hover:bg-rose-500/10 transition-colors" title="Clear all">
            ...
            {notifications.map((notification) => (
              <motion.div
                ...
                className={cn(
                  ...
                  notification.read ? 'border-white/5 opacity-60' : 'border-primary/20 bg-primary/5 shadow-lg shadow-primary/5',
                )}
              >
                {!notification.read && <div className="absolute top-0 left-0 w-1 h-full bg-primary" />}
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h3 className="font-bold text-on-surface tracking-tight">{notification.title}</h3>
