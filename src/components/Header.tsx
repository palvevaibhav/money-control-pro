    ...
    <header className="fixed top-0 w-full flex justify-between items-center px-6 h-20 bg-surface/80 backdrop-blur-xl z-50 pt-6 safe-top">
      <div className="flex items-center gap-3">
        <button 
          ...
          className="relative w-10 h-10 active:scale-95 transition-transform"
        >
          <img
            alt="User Profile"
      ...
      <div className="flex items-center gap-4">
        <button 
          ...
          className="text-primary hover:bg-white/5 transition-colors p-2 rounded-xl relative"
        >
          <Bell className="w-6 h-6" />
          {unreadCount > 0 && (
            <div className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full ring-2 ring-surface"></div>
          )}
        </button>
      </div>
    </header>
